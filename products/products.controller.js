...
const getProductByIdController = async (req, res) => { // Controlador de obtencion de producto por id.
    try {
        const { pid } = req.params // Trae el id del body. El "params" es un objeto que se envia en la peticion. Se coloca en el url. Ej. http://localhost:8080/api/products/:pid el ":pid" es un marcador de posicion, en este caso el id, por ejemplo si queremos traer el proyecto con el id 1, el ":pid" va a ser 1.

        if (!(pid && !isNaN(pid))) {
            throw { status: 400, message: "El id debe ser un numero" }
        }
        ...
        res.status(200).json({ result }) // responde con el objeto creado en el controlador de creacion de proyecto.
    }
    catch (error) {
        res.status(error.status).json({ error })
    }
...
const putProduct = async (req, res) => {
    try {
        ...
        const result = await modificarProducto(pid)
