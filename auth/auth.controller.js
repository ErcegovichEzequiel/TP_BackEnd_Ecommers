...
const registerController = async (req, res) => { // Controlador de registro de usuario
    const { email, password, passwordConfirm, edad, nombre , apellido } = req.body //desestructuracion, el body de la peticion tiene email, password, passwordConfirm, edad, nombre, apellido, que trajo del front.

    try {
        const result = await registerService({ email, password, passwordConfirm, edad, nombre, apellido }) //llamada al servicio de registro, en el servicio se hace la validacion, la validacion se hace en el helper de la carpeta utils y lo que hace es retornar un error si el usuario no cumple con las validaciones.
...
const verifyTokenController = (req, res) => {
    ...
    const esValido = jwt.verify(token, process.env.JWT_SECRET_KET)
    if (!esValido) {
