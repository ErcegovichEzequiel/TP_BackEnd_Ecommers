...
const registerService = async (usuario) => {
    try {
        const { email, password, passwordConfirm, edad, nombre, apellido } = usuario //desestructuracion de usuario
        ...
        const result = await insertarUsuario({ email: usuario.email, password: passwordHash, edad: usuario.edad, nombre: usuario.nombre, apellido: usuario.apellido }) // insertar el usuario en la base de datos
...
const loginService = async (usuario) => {
    try {
        ...
        //Controla la contraseña, compara la escrita por el usuario con la de la base de datos que tiene el hash
